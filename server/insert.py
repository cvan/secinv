...
class ServerFunctions:
    ...
    def machine(self, machine_dict, ip_dict, ports_dict):
        ...
        ## Machine.
        try:
            self.machine_obj = Machine.objects.filter(sys_ip=self.machine_ip)[0]
            self.machine_id = self.machine_obj.id
        except IndexError:
            # Add machine if not in table.
            mach_dict = {'sys_ip': self.machine_ip,
                         'hostname': machine_dict['hostname'],
                         'ext_ip': '',
        ...
        '''
        s = Services(machine=self.machine_obj,
                     processes=csv_procs,
                     ports=csv_ports)
        if s:
            print s
            s.date_updated = datetime.datetime.now()
        s.save()
        '''
        try:
            s_object = Services.objects.get(machine__id=self.machine_id)
            s_object.processes = csv_procs
            s_object.ports = csv_ports
            s_object.date_updated = datetime.datetime.now()
            s_object.save()
        ...
        for interface in ip_dict.keys():
            ...
            try:
                i_object = Interface.objects.get(machine__id=self.machine_id,
                                                 i_name=interface)
                ...
                '''
                i = Interface(machine=self.machine_obj, i_name=interface,
        ...
        '''
        print "\nInserted into assets_ip:", ip_dict
        '''
...
getattr(sf, 'machine')(machine_dict, ip_dict, ports_dict)
